...
export const totalGivenInEachMonthOfThisYear = (
  loans,
  eachLoan,
  currentUser,
  selectedYear
...
export const totalTakenInEachMonthOfThisYear = (
  loans,
  eachLoan,
  currentUser,
  selectedYear
...
export const calculateTotalForAllCategories = (expenses) => {
  ...
  const totalForACategory = function (expenses, category) {
    ...
    if (temp.length) {
      categoryTotal[category] = temp.reduce((prev, cur) => prev + cur);
    } else {
      categoryTotal[category] = 0;
    }
    return categoryTotal[category];
  };

  categories.forEach((category) => totalForACategory(expenses, category));
...
export const highestTakenAndPendingLoan = (thisUsersLoans) => {
  if (thisUsersLoans.length) {
    let loans = thisUsersLoans.filter(
      (elem) =>
        elem.value.status === "Pending" && elem.value.loanType === "Taken"
    );
