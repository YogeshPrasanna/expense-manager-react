...
class SavingsCard extends Component {
  ...
  handleClick(e) {
    ...
    if (window.confirm(message)) {
      ...
      deleteDoc(
        doc(
          firebase.db,
          `savingsTable/${this.props.authUser.uid}/savings`,
  ...
  handleSubmit(event) {
    ...
    updateDoc(
      ...
      {
        savingAmount:
          Number(this.props.savings.value.savingAmount) +
          Number(this.state.addSavingAmount),
      }
    );
  ...
  render() {
    ...
    if (!savings || !currentUser || !settings) {
      ...
      if (savings && currentUser && settings) {
        ...
        return (
          <div
            className=" col-xl-4 col-md-4 col-sm-6 col-xs-12"
            id="saving-card"
        ...
        >{savings.value.savingAmount} / {savings.value.goalAmount}</div>
            ...
            <div class="card">
              <div class="card-body">
                <h6 class="mb-4">
                  Target date: {moment(savings.value.date).format("DD/MM/yyyy")}
                </h6>
                <div class="row d-flex align-items-center">
                  <div class="col-8">
                    <h3 class="f-w-300 d-flex align-items-center m-b-0">
                      {savings.value.savingFor}
                    </h3>
                  </div>
                ...
                <div class="progress m-t-30" style={{ height: "10px" }}>
                  <div
                    ...
                    style={{
                      width: `${
                        (savings.value.savingAmount /
                          savings.value.goalAmount) *
                        100
                      }%`,
                      background: `${savings.value.cardColor}`,
                    }}
                  ></div>
                </div>
                <div class="row d-flex align-items-center">
                  <div class="col-6 text-left">
                    <p
                      class="m-b-0"
                      style={{ color: `${savings.value.cardColor}` }}
                    >
                      Saved: RM{" "}
                      {parseInt(savings.value.savingAmount).toFixed(2)}
                    </p>
                  </div>
                  <div class="col-6 text-right">
                    <p class="m-b-0">
                      Goal: RM {parseInt(savings.value.goalAmount).toFixed(2)}
                    </p>
                  </div>
                </div>
                ...
                >
                  <div className="row">
                    ...
                    >
                      ...
                      >
                        <i
                          className="fa fa-edit action-icons"
                          aria-hidden="true"
                        />
                      </button>
                      <button
