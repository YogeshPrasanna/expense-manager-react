...
class EditExpenseForm extends Component {
    constructor(props) {
        ...
        this.state = {
            date: moment(expenses.value.date),
            day: moment(expenses.value.date).day(),
            expenses: expenses.value.expenses,
            category: expenses.value.category,
            comments: expenses.value.comments,
            uid: this.props.user.uid,
            dataSaved: false,
            validationExpenses: null,
    ...
    handleSubmit(event) {
        ...
        const { expenses, comments } = this.state;
        const isInvalid = expenses === "" || comments === ""

        if (expenses === "") {
            this.setState(
              byPropKey("validationExpenses", "Please enter expense value")
            );
        } else {
            this.setState(byPropKey("validationExpenses", null));
        }
        ...
        if(!isInvalid) {
            updateDoc(doc(firebase.db, `expenseTable/${this.props.user.uid}/expenses`, this.props.expenses.key), {
    ...
    render() {
        if (this.props.settings) {
            ...
            const { validationExpenses, validationComments } = this.state;
