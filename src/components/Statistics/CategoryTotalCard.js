...
class CategoryTotalCard extends Component {
  ...
  render() {
    ...
    if (expenses && currentUser && selectedYear && settings) {
      ...
      if (selectedYear == "all") {
        allCategoryTotals =
          utils.calculateTotalForAllCategories(thisUsersExpenses);
      } else {
      ...
      categoryList = eachCategory(allCategoryTotals).map((el, i) => {
        let catName = settings.editedCategories[el.key]
          ? settings.editedCategories[el.key]
          : el.key;
        if (el.value) {
          if (selectedYear === "all") {
            return (
            ...
            return (
              <Link
                key={i}
                to={`/filter-view?category=${el.key}&selectedYear=${selectedYear}&from=yearpage`}
              >
                <span style={category} className="ttt" key={el.key}>
                  <div style={utils.categoryName(el.key, "card")}>
                    {catName}
                  </div>
                  <i
                    className={`fa fa-${utils.categoryIcon(el.key)}`}
